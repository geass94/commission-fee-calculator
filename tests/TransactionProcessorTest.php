...
class TransactionProcessorTest extends KernelTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->binProvider = $container->get(BinProviderService::class);
        $this->binProvider->setHttpClient($this->mockBinApiClient());
        $this->rateProvider = $container->get(ExchangeRateService::class);
        $this->rateProvider->setHttpClient($this->mockRateApiClient());
        $this->transactionProcessor = $container->get(TransactionProcessorImpl::class);
        $this->transactionProcessor->setBinProvider($this->binProvider);
        $this->transactionProcessor->setRateProvider($this->rateProvider);
    }

    public function test_getBinCountry_returns_country_class_by_given_bin_number()
    {
        $this->binProvider->setHttpClient($this->mockBinApiClient());
        $provider = $this->binProvider;
    ...
    public function test_getBinCountry_returns_503_when_remote_service_fails()
    {
        ...
        $this->binProvider->setHttpClient($client);
        $provider =$this->binProvider;
